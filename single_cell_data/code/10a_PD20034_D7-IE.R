...
# Import data
h5_file = paste0("../data/gexp/ALLO_Sample34-Day7_rna_filtered_feature_bc_matrix.h5")
...
# Append kite counts
hhv6_mat <- import_from_kite_matrix(unique(fread(paste0("../data/feature_counts/ALLO_Sample34-Day7_HHV6b.kb.txt.gz"))),
...
ggplot(df, aes(x = cor.immediate_early, y = cor.intermediate_early_early)) +
  geom_point() + pretty_plot(fontsize = 20) + L_border()+  
  labs(x = "Cor. w/ HHV6 late genes", 
       y = "Cor. w/ HHV6 early genes")

df %>% arrange(cor.immediate_early) %>% head()
