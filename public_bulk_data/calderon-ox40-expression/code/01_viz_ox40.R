...
order_df <- data.frame(sn = colnames(dt)[-1], OX40 = round(mat[gene,]/colSums(mat) * 1000000, 1)) %>%
  arrange(desc(OX40)) %>%
  mutate(rank = 1:n())
order_df
# Append meta data
...
p1 <- ggplot(modf %>% arrange(rank), aes(x = rank2, y = OX40, color = lineage)) + 
  geom_point(size = 0.5) + scale_color_manual(values = jdb_palette("corona")[c(3,1,2,4,5,6)]) +
  pretty_plot() + labs(x = "Rank ordered expression", y = "OX40 transcripts per million") +
  facet_wrap(~stimstim)
p1
